...
export async function runJava(problem: Problem, userCode: string): Promise<ExecutionResponse> {
  ...
  try {
    ...
    console.log(`param type inside full code: ${paramType}`);
    ...
    // Add JS-side logs for each test case
    for (let i = 0; i < testCases.length; i++) {
      ...
      console.log(`[GEN][Test ${i + 1}] paramType:`, paramType);
      console.log(`[GEN][Test ${i + 1}] input:`, input);
      console.log(`[GEN][Test ${i + 1}] cleanInput:`, cleanInput);
      // Note: parsedInput is determined in the Java code, but you can log what it would be for String
      if (paramType === 'String') {
        console.log(`[GEN][Test ${i + 1}] would use parsedInput:`, cleanInput);
      }
    }
